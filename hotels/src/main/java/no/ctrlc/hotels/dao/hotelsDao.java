...
public class hotelsDao {
    ...
    public Hotel getHotelById( Integer hotelId) {
        ...
        SqlParameterSource namedParameters = new MapSqlParameterSource("id", id); // samme som
                                                                                  // MapSqlParameterSource().add("id",id);
    ...
    public List<Customer> getCustomerByPhoneNumber( String customerPhoneNumber) {
        SqlParameterSource namedParameters = new MapSqlParameterSource("phoneNumber", customerPhoneNumber); // samme som
                                                                                                            // MapSqlParameterSource().add("id",id);
    ...
    public List<Customer> getCustomerByEmailAddress( String emailAddress) {
        SqlParameterSource namedParameters = new MapSqlParameterSource("emailAddress", emailAddress); // samme som
                                                                                                      // MapSqlParameterSource().add("id",id);
    ...
    public List<Room> getAvailableRooms( Integer hotelId, LocalDate fromDate, LocalDate toDate) {
        // rooms have hotel+roomnumber as primary keys. We need to get the available
        // roomnumbers;
        final String sql = "SELECT *" + " FROM rooms r" + " WHERE r.hotelid = '" + hotelId + "'"
                + " AND r.roomnumber NOT IN" + " ( SELECT o.roomnumber" + " 	FROM orders o" + " 	WHERE o.hotelid = '"
                + hotelId + "'" + " 	AND ( DATEDIFF('" + fromDate + "',o.todate) < 0 AND DATEDIFF('" + toDate
                + "',o.fromdate) > 0 )" + " )";
    ...
    private Room getRoom( int hotelId, int roomNumber) {
        final String sql = "SELECT *" + " FROM rooms r" + " WHERE r.hotelid = " + hotelId + " AND r.roomnumber = "
                + roomNumber;
        Room room = null;
        try {
            ...
            return null;
        } // return null if query did not give exactly 1 result
        return room;
    }
