...
public class RoomController {
	...
	public String roomSearch(
			...
			Model model) {
		// if we do not have a valid hotel and valid dates, we can't search for an available room. 
		// Return to home.
		if ( bindingResult.hasErrors() ){
			model.addAttribute("error.preRequirement", new String("Choose hotel and dates before searching for a room"));
			return "home";
		} else {
			// get list with available rooms
			ArrayList<Room> availableRooms = (ArrayList<Room>) hotelsService.getAvailableRooms(order.getHotel().getId(), order.getDates().getFromDate(), order.getDates().getToDate());			
	...
	public String roomSelect(
		...
		// remove list of availableRooms from Model as it's no longer needed in the model,
		// and it should be retrieved from database if needed the database state might change.
	...
	public String showRoomDetails(
		...
		BindingResult bindingResult,
		Model model) {
		// if we do not have a valid hotel and valid dates, do not show details for a room.
		// Return to home page.
		if ( bindingResult.hasErrors() ){
			model.addAttribute("error.preRequirement", new String("Choose hotel and dates before trying to show the rooms details"));
			return "home";
		}
			// if we don't have a room we can't show the details
		if (order.room == null) { 
			model.addAttribute("error.preRequirement", new String("Choose a room before trying to show the rooms details"));
			return "home";
		}
		// return view
