...
public class Customer {
	...
	@Size(
		min=6, 
		max=100, 
		message="Address should be between 6 and 100 characters", 
		groups=FirstStep.class 
	)
	private String address;

	@Email(message = "Email address must be valid", groups=FirstStep.class )
	@NotBlank(message = "Email address must not be null or blank", groups=FirstStep.class ) // @Email allows null and blank as valid email so we need to set this.
    private String emailAddress;
	
	@Pattern(
		regexp="(^$|^(\\+)?\\d{3,15}$)", 
		message = "Phonenumber should be blank or an optional + and between 3 and 15 digits",
		groups=FirstStep.class
	)
