...
public class Hotel implements Comparable<Hotel> {
    ...
    private @NotNull(message = "Hotelid Cannot be null") @Min(value = 1, message = "Hotel id must be between 1 and 99") @Max(value = 99, message = "Hotel id must be between 1 and 99") Integer id;
    ...
    @Override
    public int compareTo( final Hotel otherHotel) throws NullPointerException, ClassCastException {
        if (otherHotel == null) {
            throw new NullPointerException();
        } else if (otherHotel instanceof Hotel == false) {
            throw new ClassCastException();
        } else {
            return id.compareTo(otherHotel.getId());
        }
    }
}